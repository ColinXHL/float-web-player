...
public class PluginApiTests : IDisposable
{
    ...
    {
        return new PluginManifest { Id = id, Name = "Test Plugin", Version = "1.0.0", Main = "main.js",
...
#region Property 1 : 沙箱安全性
    ...
    public Property AuthorizedApiAccess_ShouldReturnNonNull(PositiveInt permIndex)
    {
        ...
        bool authorizedApiReturnsNonNull = authorizedPermission switch { "overlay" => api.Overlay != null,
                                                                         _ => true };
...
#region Property 1 : 权限控制一致性(新增 API)
    ...
    {
        // 新增的需要权限的 API 及其对应权限
        var permissionApiPairs = new[] { ("player", new Func<PluginApi, object?>(api => api.Player)),
        ...
        return (hasPermissionResult && apiWithPermissionNotNull && noPermissionResult && apiWithoutPermissionIsNull &&
    ...
    public Property PermissionControlConsistency_MultiplePermissions(PositiveInt seed)
    {
        ...
        // 验证 HasPermission 方法
        var hasPermissionCorrect = allNewPermissions.All(p => api.HasPermission(p) == grantedPermissions.Contains(p));
    ...
    public Property NoPermissionApis_AlwaysAvailable(PositiveInt seed)
    {
        // 随机生成权限列表
        var allPermissions =
            new[] { "player", "window", "storage", "network", "events", "audio", "overlay", "subtitle" };
        var grantedPermissions = new List<string>();
        var seedValue = seed.Get;
