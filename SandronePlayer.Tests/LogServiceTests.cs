...
namespace SandronePlayer.Tests
{
    /// <summary>
    /// LogService 属性测试
    /// </summary>
    public class LogServiceTests
    {
        ...
        /// *对于任意*日期，生成的日志文件名应匹配模式 sandrone-player-yyyyMMdd.log
        ...
        public Property LogFileName_ShouldMatchPattern(int year, int month, int day)
        {
            ...
            // 验证文件名格式
            var pattern = @"^sandrone-player-\d{8}\.log$";
            var matchesPattern = Regex.IsMatch(fileName, pattern);

            // 验证日期部分正确
            var expectedFileName = $"sandrone-player-{date:yyyyMMdd}.log";
