...
#region Private Methods
    ...
    private object ExecuteRequest(HttpMethod method, string url, object? body, object? options)
    {
        try
        {
            ...
            // 返回一致的响应结构：{ success, status, data, headers, error? }
            return new { success = true, status = (int)response.StatusCode, data = responseBody,
                         headers = responseHeaders, error = (string?)null };
        }
    ...
    {
        return new { success = false, status = 0, data = (string?)null, headers = new Dictionary<string, string>(),
                     error = error };
    }
