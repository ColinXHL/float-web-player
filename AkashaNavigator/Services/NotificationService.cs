...
#region Public Methods
    ...
    public void Show(string message, NotificationType type = NotificationType.Info, string? title = null,
                     int durationMs = 3000)
    {
        try
        {
            ...
            Application.Current.Dispatcher.Invoke(() =>
                                                  {
                                                      var config = new NotificationConfig { Message = message,
    ...
    public Task<bool?> ShowDialogAsync(string message, string? title = null, string yesText = "确定",
...
#region Convenience Methods
    ...
    public void Info(string message, string? title = null,
                     int durationMs = 3000) => Show(message, NotificationType.Info, title, durationMs);

    /// <summary>
    /// 显示成功通知
    /// </summary>
    public void Success(string message, string? title = null,
                        int durationMs = 3000) => Show(message, NotificationType.Success, title, durationMs);

    /// <summary>
    /// 显示警告通知
    /// </summary>
    public void Warning(string message, string? title = null,
                        int durationMs = 3000) => Show(message, NotificationType.Warning, title, durationMs);

    /// <summary>
    /// 显示错误通知
    /// </summary>
    public void Error(string message, string? title = null,
                      int durationMs = 4000) => Show(message, NotificationType.Error, title, durationMs);
