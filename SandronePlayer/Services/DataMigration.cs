...
namespace SandronePlayer.Services
{
    ...
    public class MigrationResult
    {
        ...
        public bool IsSuccess => Status == MigrationResultStatus.Success || 
                                  Status == MigrationResultStatus.NotNeeded;

        public static MigrationResult Success(int pluginCount, int profileCount) => new()
        {
            Status = MigrationResultStatus.Success,
            MigratedPluginCount = pluginCount,
            MigratedProfileCount = profileCount
        };

        public static MigrationResult NotNeeded() => new()
        {
            Status = MigrationResultStatus.NotNeeded
        };

        public static MigrationResult Failed(string message) => new()
        {
        ...
        {
            Status = MigrationResultStatus.PartialSuccess,
            MigratedPluginCount = pluginCount,
            MigratedProfileCount = profileCount,
            Warnings = warnings
        };
    ...
    public class DataMigration
    {
        ...
        public Dictionary<string, List<OldPluginInfo>> GetOldStylePlugins()
        {
            ...
            foreach (var profileDir in Directory.GetDirectories(ProfilesDirectory))
            {
                ...
                foreach (var pluginDir in Directory.GetDirectories(pluginsDir))
                {
                    ...
                    plugins.Add(new OldPluginInfo
                    {
                        PluginId = manifestResult.Manifest.Id ?? pluginDirName,
        ...
        public MigrationResult Migrate()
        {
            ...
            try
            {
                ...
                foreach (var kvp in uniquePlugins)
                {
                    ...
                    try
                    {
                        ...
                        // 添加到索引
                        libraryIndex.Plugins.Add(new InstalledPluginEntry
                        {
                            Id = pluginId,
                            Version = pluginInfo.Manifest?.Version ?? "1.0.0",
                ...
                LogService.Instance.Info("DataMigration", 
            ...
            catch (Exception ex)
            {
                ...
                if (backup != null)
                {
                    ...
                    {
                        Rollback(backup);
                        LogService.Instance.Info("DataMigration", "已回滚迁移更改");
                    }
        ...
        public void Rollback(MigrationBackup backup)
        {
            ...
            LogService.Instance.Info("DataMigration", "迁移回滚完成");
        }
