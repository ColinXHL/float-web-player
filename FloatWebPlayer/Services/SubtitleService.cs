...
(async function() {
    try {
        ...
        console.log('[FloatPlayer] 视频信息: bvid=' + bvid + ', aid=' + aid);
        ...
        console.log('[FloatPlayer] 请求字幕 API: ' + apiUrl);
        ...
        console.log('[FloatPlayer] API 返回 cid: ' + cid);

        const subtitles = data?.data?.subtitle?.subtitles || [];
        console.log('[FloatPlayer] 找到 ' + subtitles.length + ' 个字幕');
        ...
        console.log('[FloatPlayer] 选择字幕: ' + selectedSubtitle.lan + ', URL: ' + subtitleUrl);
        ...
        console.error('[FloatPlayer] 获取字幕失败:', error);
        window.chrome.webview.postMessage(JSON.stringify({
            type: 'subtitle_error',
